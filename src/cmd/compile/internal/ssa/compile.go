...
var passes = [...]pass{
	...
	// Loop Optimizations
	{name: "loop deadcode", fn: deadcode, required: true},    // remove dead blocks before loop opts to avoid extra work
	{name: "loop invariant code motion", fn: licm},           // hoist loop invariant code out of loops
	{name: "lcssa destruct", fn: phielim},                    // eliminate LCSSA proxy phi to restore general SSA form
	{name: "loop sccp", fn: sccp},                            // optimize loop guard conditional test
	{name: "loop opt", fn: opt},                              // further optimize loop guard conditional test
	{name: "loop deadcode late", fn: deadcode},               // remove dead loop guard to simplify cfg
