...
// Simple statistics show that 60% of LICM candidate loops contain at least one
// function call, in the absence of inter-procedural modref analysis we will
// exclude some intrinsic functions as much as possible because we know exactly
// how they behave, for the rest of the function calls , we are not sure if they
// affect the correctness of TBAA so we conservatively abstain from continuing
// the optimization
var safeRuntimeCalls = []string{
	"runtime.growslice",
	"runtime.newobject",
	"runtime.decoderune",
	"runtime.makeslice",
...
func isSafeCall(call *Value) bool {
	assert(call.Op.IsCall(), "sanity check")
	fn := call.Aux.(*AuxCall).Fn
	if fn == nil {
		return false
	}
	fnName := fn.String()
	for _, safe := range safeRuntimeCalls {
		if safe == fnName {
			return true
		}
	}
...
func licm(fn *Func) {
	// if fn.Name != "plugin_lastmoduleinit" {
	// 	return
	// }
